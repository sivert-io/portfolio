...
function getTooltipPosition(x: number, y: number) {
  ...
  if (left + TOOLTIP_WIDTH + VIEWPORT_PADDING > viewportWidth) {
    left = x - TOOLTIP_WIDTH - TOOLTIP_GAP
  }
...
export function CareerTimeline({ items }: CareerTimelineProps) {
  ...
  const tooltipPosition =
    hovered && typeof window !== 'undefined' ? getTooltipPosition(hovered.x, hovered.y) : null

  return (
    <div className="not-prose relative flex flex-col gap-10 py-6">
        ...
        <div className="grid gap-10">
          {groupedItems.map((group) => (
            <section key={group.year} className="grid gap-6 md:grid-cols-[96px_1fr]">
              <div className="flex items-start py-2 text-sm font-semibold tracking-[0.18em] text-white/35 uppercase">
                {group.year}
              </div>
      ...
      <AnimatePresence>
        {hovered && tooltipPosition ? (
          <motion.div
            ...
            className="pointer-events-none fixed z-[100] flex w-[280px] flex-col gap-3 rounded-2xl border border-white/10 bg-black/85 p-4 shadow-2xl backdrop-blur-xl"
            style={{
          ...
          >
            ...
            {hovered.item.subtitle ? (
              <p className="text-xs text-white/55">{hovered.item.subtitle}</p>
            ) : null}
            ...
            {hovered.item.tooltip?.footer ? (
              <p className="text-xs leading-relaxed text-white/40">{hovered.item.tooltip.footer}</p>
            ) : null}
          </motion.div>
        ) : null}
      </AnimatePresence>
        ...
        {selectedItem ? (
          <>
            ...
            <motion.aside
              ...
              className="fixed inset-x-4 top-1/2 z-[120] mx-auto flex w-full max-w-2xl -translate-y-1/2 flex-col gap-6 rounded-3xl border border-white/10 bg-zinc-950/95 p-6 shadow-2xl backdrop-blur-xl"
            >
                ...
                <div className="flex min-w-0 items-start gap-4">
                  ...
                  <div className="flex min-w-0 flex-col gap-3">
                    ...
                    {selectedItem.subtitle ? (
                      <p className="text-sm text-white/58">{selectedItem.subtitle}</p>
                    ) : null}
